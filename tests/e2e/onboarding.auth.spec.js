...
test('browser OTP login, real uploads, rejection, replacement, resubmission and approval', async ({ browser }, testInfo) => {
  ...
  try {
    ...
    await cards.first().getByRole('button', { name: 'Replace document', exact: true }).click()
    await cards.first().locator('input[type="file"]').setInputFiles({ name: 'fictional-offline-replacement.pdf', mimeType: 'application/pdf', buffer: Buffer.from('%PDF-1.4\nFictional offline retry test\n%%EOF') })
    await applicant.context().setOffline(true)
    await cards.first().getByRole('button', { name: 'Upload', exact: true }).click()
    await expect(cards.first().getByText('Replacement is not confirmed. Retry the upload before submitting.', { exact: true })).toBeVisible()
    await expect(applicant.getByRole('button', { name: 'Submit for verification', exact: true })).toBeDisabled()
    await expect(cards.first().getByRole('button', { name: 'Keep current document', exact: true })).toHaveCount(0)
    await applicant.context().setOffline(false)
    ...
    expect(uploads.length).toBe(application.documents.length + 2) // includes the real offline PUT attempt
